...
class MusicXMLParserState(object):
    ...
    def __init__(self):
        ...
        self.velocity = constants.DEFAULT_MIDI_VELOCITY

        # Default MIDI program (0 = grand piano)
        self.midi_program = constants.DEFAULT_MIDI_PROGRAM

        # Current MIDI channel (usually equal to the part number)
        self.midi_channel = constants.DEFAULT_MIDI_CHANNEL
...
class MusicXMLDocument(object):
    ...
    If the file is .mxl, this class un-compresses it

    After the file is loaded, this class then parses the document into memory
    using the parse method.
    """

    def __init__(self, file_content: bytes, is_compressed: bool):
        self._score = self._get_score(file_content, is_compressed)
        self.parts = []
    ...
    @staticmethod
    def _get_score(file_content: bytes, is_compressed: bool):
        """ Given a MusicXML file, return the score as a xml.etree.ElementTree.

        Given a MusicXML file, return the score as a xml.etree.ElementTree
        If the file is compress (ends in .mxl), uncompress it first

        Args:
            file_content: The content (bytes) of a MusicXML file
            is_compressed: Whether the file is compressed (MXL format) or not

        Returns:
          The score as a xml.etree.ElementTree.

        Raises:
          MusicXMLParseError: if the file cannot be parsed.
        """
        musicxml_file = io.BytesIO(file_content)
        if is_compressed:
            # Compressed MXL file. Uncompress in memory.
            try:
                mxlzip = zipfile.ZipFile(musicxml_file)
            except zipfile.BadZipfile as exception:
                raise exceptions.MusicXMLParseError(exception)

            # A compressed MXL file may contain multiple files, but only one
            # MusicXML file. Read the META-INF/container.xml file inside the
                        ...
                        if 'media-type' in rootfile_tag.attrib:
                            if rootfile_tag.attrib['media-type'] == MUSICXML_MIME_TYPE:
                                if not compressed_file_name:
                                    ...
                                    raise exceptions.MusicXMLParseError('Multiple MusicXML files found in compressed'
                                                                        'archive')
                        else:
            ...
            if not compressed_file_name:
                raise exceptions.MusicXMLParseError('Unable to locate main .xml file in compressed archive.')
            try:
                compressed_file_info = [x for x in infolist if x.filename == compressed_file_name][0]
            except IndexError:
                raise exceptions.MusicXMLParseError('Score file %s not found in zip archive' % compressed_file_name)
    ...
    def get_time_signatures(self):
        ...
        Example: If Part 1 has a time signature of 4/4 and Part 2 also
...
class Part(object):
    """Internal representation of a MusicXML <part> element."""
...
class Measure(object):
    """Internal representation of the MusicXML <measure> element."""
        ...
        for child in xml_attributes:
            if child.tag == 'divisions':
                ...
                if self.time_signature is None:
                    ...
                    raise exceptions.MultipleTimeSignatureError('Multiple time signatures')
...
class Note(object):
    ...
    def _parse(self):
        ...
        for child in self.xml_note:
            if child.tag == 'chord':
                ...
                raise exceptions.UnpitchedNoteError('Unpitched notes are not supported')
            else:
                # Ignore other tag types because they are not relevant to Magenta.
                pass
    ...
    def pitch_to_midi_pitch(step, alter, octave):
        ...
        if step == 'C':
            ...
            raise exceptions.PitchStepParseError('Unable to parse pitch step ' + step)
...
class NoteDuration(object):
    ...
    def type(self, new_type):
        if new_type not in self.TYPE_RATIO_MAP:
            raise exceptions.InvalidNoteDurationTypeError('Note duration type "{}" is not valid'.format(new_type))
        self._type = new_type
...
class ChordSymbol(object):
    ...
    def _alter_to_string(self, alter_text):
        ...
        try:
            ...
            raise exceptions.ChordSymbolParseError('Non-integer alter: ' + str(alter_text))
        ...
        if alter_semitones == -2:
            ...
            raise exceptions.ChordSymbolParseError('Invalid alter: ' + str(alter_semitones))
    ...
    def _parse(self):
        ...
        for child in self.xml_harmony:
            if child.tag == 'root':
                ...
                if kind_text not in self.CHORD_KIND_ABBREVIATIONS:
                    raise exceptions.ChordSymbolParseError('Unknown chord kind: ' + kind_text)
                ...
                try:
                    ...
                    raise exceptions.ChordSymbolParseError('Non-integer offset: ' + str(child.text))
        ...
        if self.root is None and self.kind != 'N.C.':
            raise exceptions.ChordSymbolParseError('Chord symbol must have a root')
    ...
    def _parse_pitch(self, xml_pitch, step_tag, alter_tag):
        ...
        if xml_pitch.find(step_tag) is None:
            raise exceptions.ChordSymbolParseError('Missing pitch step')
        ...
        if self.state.transpose:
            raise exceptions.ChordSymbolParseError('Transposition of chord symbols currently unsupported')
    ...
    def _parse_degree(self, xml_degree):
        ...
        if xml_degree.find('degree-value') is None:
            raise exceptions.ChordSymbolParseError('Missing scale degree value in harmony')
        value_text = xml_degree.find('degree-value').text
        if value_text is None:
            raise exceptions.ChordSymbolParseError('Missing scale degree')
        try:
            value = int(value_text)
        except ValueError:
            raise exceptions.ChordSymbolParseError('Non-integer scale degree: ' + str(value_text))
        ...
        if xml_degree.find('degree-type') is None:
            raise exceptions.ChordSymbolParseError('Missing degree modification type')
        ...
        if type_text == 'add':
            ...
            if not alter_string:
                raise exceptions.ChordSymbolParseError('Degree alteration by zero semitones')
            # No type string necessary as merely appending e.g. "#9" suffices.
            type_string = ''
        else:
            raise exceptions.ChordSymbolParseError('Invalid degree modification type: ' + str(type_text))
...
class TimeSignature(object):
    ...
    def _parse(self):
        ...
        if (len(self.xml_time.findall('beats')) > 1 or
            ...
            raise exceptions.AlternatingTimeSignatureError('Alternating Time Signature')
        ...
        try:
            ...
            raise exceptions.TimeSignatureParseError('Could not parse time signature: {}/{}'.format(beats, beat_type))
        self.time_position = self.state.time_position
...
class KeySignature(object):
    ...
    def _parse(self):
        ...
        if fifths is None:
            raise exceptions.KeyParseError('Could not find fifths attribute in key signature.')
...
def musicxml_to_note_sequence(musicxml_file: bytes, source_type: str = 'mxml'):
    ...
    sequence.source_info.source_type = NoteSequence.SourceInfo.SCORE_BASED
    sequence.source_info.encoding_type = NoteSequence.SourceInfo.MUSIC_XML
    sequence.source_info.parser = NoteSequence.SourceInfo.MAGENTA_MUSIC_XML
        ...
        for musicxml_measure in musicxml_part.measures:
            for musicxml_note in musicxml_measure.notes:
                if not musicxml_note.is_rest:
                    ...
                    duration_ratio = musicxml_note.note_duration.duration_ratio()
                    note.numerator = duration_ratio.numerator
                    note.denominator = duration_ratio.denominator
...
def musicxml_file_to_note_sequence(musicxml_file_path: str):
    ...
    try:
        with open(musicxml_file_path, 'r') as f:
            musicxml_file = f.read()
            musicxml_document = MusicXMLDocument(musicxml_file, musicxml_file_path.endswith(".mxl"))
    except exceptions.MusicXMLParseError as e:
        raise exceptions.MusicXMLConversionError(e)
    return musicxml_to_note_sequence(musicxml_document)
